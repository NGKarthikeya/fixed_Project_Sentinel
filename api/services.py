...
# Optional import — only used at startup; keeps module load-time fast.
try:
    from dataset_seeder import seed_all as _seed_datasets
...
async def startup() -> None:
    ...
    # ── Dataset seeding ─────────────────────────────────────────────
    # Pre-populate Redis (or in-memory fallback) with bundled security
    # datasets so scenarios have a realistic threat-intel baseline.
    if _SEEDER_AVAILABLE and config.dataset_seed_on_startup:
        try:
            seed_results = _seed_datasets(dry_run=False)
            logger.info(logfmt("dataset_seed_complete", results=seed_results))
        except Exception as _seed_exc:
            # Never block startup due to seeding failures
            logger.warning(logfmt("dataset_seed_failed", error=str(_seed_exc)))
    elif not config.dataset_seed_on_startup:
        logger.info(logfmt("dataset_seed_skipped", reason="DATASET_SEED_ON_STARTUP=false"))
    # ────────────────────────────────────────────────────────────────
