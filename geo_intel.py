...
DEFAULT_TRUSTED_MTAS = [r".*\.company\.com$", r".*\.security\.net$"]
DEFAULT_TOR_NODES = {"185.220.101.5", "185.220.101.4", "185.220.101.6"}
DEFAULT_HOSTING_ASNS = {"AS24940", "AS16509", "AS14061"}
DEFAULT_HOSTING_ISPS = ["Tor Exit Node Network", "AWS", "Amazon", "DigitalOcean", "Hetzner", "Linode"]
DEFAULT_VPN_PROVIDERS = ["NordVPN", "ExpressVPN", "Mullvad", "ProtonVPN"]
...
def is_trusted_mta(identifier: str) -> bool:
    ...
    if compiled_patterns:
        ...
        patterns = getattr(config, "TRUSTED_MTAS", DEFAULT_TRUSTED_MTAS)
        for pattern in patterns:
            if re.search(pattern, identifier_clean, re.IGNORECASE):
                return True
    return False
...
def flag_anonymization(ip: str, isp: str, asn: str) -> Dict[str, bool]:
    ...
    tor_nodes = getattr(config, "KNOWN_TOR_NODES", DEFAULT_TOR_NODES)
    hosting_asns = getattr(config, "KNOWN_HOSTING_ASNS", DEFAULT_HOSTING_ASNS)
    hosting_isps = getattr(config, "KNOWN_HOSTING_ISPS", DEFAULT_HOSTING_ISPS)
    vpn_providers = getattr(config, "KNOWN_VPN_PROVIDERS", DEFAULT_VPN_PROVIDERS)

    is_tor = ip in tor_nodes
    is_hosting = asn in hosting_asns
    if not is_hosting:
        for hosting_isp in hosting_isps:
            if hosting_isp.lower() in isp.lower():
                is_hosting = True
                break
    is_vpn = False
    for vpn_pattern in vpn_providers:
